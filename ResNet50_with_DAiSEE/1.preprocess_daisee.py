...
class VideoPreprocessor:
    ...
    def process_video_worker(args):
        row, split = args  # Unpack only necessary arguments

        try:
            # Create a temporary instance to access attributes
            temp_processor = VideoPreprocessor()
            ...
            video_path = os.path.join(temp_processor.dataset_dir, split, clip_id[:6], clip_id[:-4], clip_id)
            output_folder = os.path.join(temp_processor.output_base_dir, split, label, clip_id[:-4])

            if not os.path.exists(video_path):
                return None

            frames_count = VideoPreprocessor.extract_frames(
                video_path, output_folder, temp_processor.cascade_path,
                use_fast_detector=temp_processor.use_fast_detector, target_size=temp_processor.target_size
            )
